                ...
                if (action != null) {
                    if(action.equals("verifyPwd")) {
                        ...
                        if(inputPwd == null || inputPwd.trim().equals("")) {
                            resp.getWriter().write("Mật khẩu không được trống!");
                        }
                        else {
                           boolean isPwdCorrect =  UserService.getInstance().verifyPwd(user.getId() + "", inputPwd);
                            if (isPwdCorrect) {
                                resp.getWriter().write("Mật khẩu chính xác!");
                            } else {
                                resp.getWriter().write("Mật khẩu không đúng, vui lòng thử lại.");
                            }
                        }
                    }
